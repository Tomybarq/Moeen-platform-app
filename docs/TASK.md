...
## 🔷 أمن مستوى الصف والإحصائيات الآمنة (RLS & Secure Stats API) - [مكتملة]
- `[x]` إنشاء نظام أمن مستوى الصف (Row-Level Security):
  - `[x]` إنشاء ملف الموزع المركزي للأذونات وعقد الفلترة `src/lib/auth-rls.ts`.
  - `[x]` ربط قيود الأدوار (Charity Staff, Marketer, Social Researcher, etc) في Prisma.
- `[x]` إنشاء نقطة وصول الإحصائيات المؤمّنة:
  - `[x]` إنشاء `src/app/api/portal/stats/route.ts` مع فحص الجلسة.
  - `[x]` استخدام RLS وتمرير الفلاتر في استعلامات Prisma بالتوازي لحساب المستفيدين النشطين، طلبات الدعم، والجمعيات الخيرية.

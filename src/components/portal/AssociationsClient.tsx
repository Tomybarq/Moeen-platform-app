...
export default function AssociationsClient({
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.checked) {
      setSelectedIds(associations.map((a) => a.id));
    } else {
      setSelectedIds([]);
    }
  };
  ...
  const handleAddClick = () => {
    showToast(isAr ? "تم النقر على زر إضافة جمعية جديدة" : "Clicked add new association button", "info");
  };

  const handleEditClick = (id: number, name: string) => {
    showToast(isAr ? `تعديل الجمعية: ${name} (ID: ${id})` : `Editing association: ${name} (ID: ${id})`, "info");
  };
      ...
      <div>
        ...
        <div className="flex items-center gap-3 mt-4">
          {hasPermission(path, "create") && (
            <AddButton
              label={locale === "ar" ? "إضافة جمعية" : "Add Association"}
                ...
                <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-[#1E293B]/50 text-slate-500 dark:text-slate-400 text-xs font-semibold uppercase">
                  <th className="p-4 text-start w-12">
                    <input
                      type="checkbox"
                      checked={
                        selectedIds.length === associations.length && associations.length > 0
                      }
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800 text-sm">
                {associations.map((assoc) => (
                  ...
                  >
                    ...
                    <td className="p-4">{assoc.id}</td>
                    <td className="p-4 font-medium text-slate-900 dark:text-white">
                      {assoc.name}
                    </td>
                    <td className="p-4">{new Date(assoc.createdAt).toLocaleDateString()}</td>
                    <td className="p-4 text-end">
                      {hasPermission(path, "edit") && (
                        <EditButton
                          label={locale === "ar" ? "تعديل" : "Edit"}
                          onClick={() => handleEditClick(assoc.id, assoc.name)}
                        />
                      )}
                    </td>
                  </tr>

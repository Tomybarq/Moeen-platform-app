...
export default function AssociationsClient({ locale }: AssociationsClientProps) {
  ...
  // Delete state
  const [deleteItem, setDeleteItem] = useState<Association | null>(null);
  ...
  const fetchData = useCallback(async (q: string, p: number) => {
    setLoading(true);
    try {
      const response = await fetch(`/api/associations?search=${encodeURIComponent(q)}&page=${p}&limit=${LIMIT}`);
  ...
  const handleNameBlur = () => {
    if (formName.trim().length < 3) {
      setFormNameError(
        isAr ? "اسم الجمعية يجب أن يكون 3 أحرف على الأقل" : "Association name must be at least 3 characters"
      );
    } else {
      setFormNameError("");
    }
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const url = editItem ? `/api/associations/${editItem.id}` : "/api/associations";
      ...
      if (response.ok) {
        showToast(
          isAr
            ? editItem
              ? "تم تعديل الجمعية بنجاح"
              : "تم إضافة الجمعية بنجاح"
            : editItem
  ...
  // ── Delete ──────────────────────────────────────────────────────────────────
  const handleOpenDelete = (item: Association) => {
    setDeleteItem(item);
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/associations/${deleteItem.id}`, {
        method: "DELETE",
      });
      const data = await response.json();

      if (response.ok) {
        showToast(isAr ? "تم حذف الجمعية بنجاح" : "Association deleted successfully", "success");
        setDeleteItem(null);
        // If we deleted the last item on the page, go to previous page
        const newPage = associations.length === 1 && page > 1 ? page - 1 : page;
        setPage(newPage);
        fetchData(search, newPage);
      } else {
        showToast(data.error || (isAr ? "فشل حذف الجمعية" : "Failed to delete association"), "error");
      }
    } catch (err) {
      showToast(isAr ? "خطأ في الاتصال بالخادم" : "Server connection error", "error");
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-white">
            {isAr ? "إدارة الجمعيات" : "Manage Associations"}
          </h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            {isAr ? `إجمالي الجمعيات: ${total}` : `Total Associations: ${total}`}
          </p>
        </div>
        {hasPermission(PATH, "create") && (
          <AddButton
            label={isAr ? "إضافة جمعية" : "Add Association"}
      ...
      <div className="flex flex-col sm:flex-row gap-4 bg-white dark:bg-[#0F172A] border border-slate-200 dark:border-slate-800 p-4 rounded-2xl shadow-sm">
        <SearchInput
          ...
          placeholder={isAr ? "البحث عن جمعية..." : "Search associations..."}
        ...
        {loading ? (
          ...
          <div className="bg-white dark:bg-[#0F172A] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm">
            <EmptyState
              ...
              description={isAr ? "لم نجد أي جمعية تطابق معايير بحثك." : "We couldn't find any associations matching your query."}
            />
          </div>
        ) : (
              ...
              <table className="w-full text-start border-collapse">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/20 text-slate-500 dark:text-slate-400 text-xs font-bold">
                    ...
                    <th className="p-4 text-start">{isAr ? "الجمعية" : "Association"}</th>
                      ...
                      <td className="p-4 text-end">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          {hasPermission(PATH, "delete") && (
                            <button
                              onClick={() => handleOpenDelete(assoc)}
        ...
        <form onSubmit={handleFormSubmit} className="p-6 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder={isAr ? "مثال: جمعية معين الأهلية" : "e.g. Moeen Charity"}
      ...
      <ConfirmDialog
        ...
        message={
          isAr
            ? `هل أنت متأكد من رغبتك في حذف جمعية "${deleteItem?.name}"؟ لا يمكن التراجع عن هذا الإجراء.`
            : `Are you sure you want to delete association "${deleteItem?.name}"? This action cannot be undone.`
        }
